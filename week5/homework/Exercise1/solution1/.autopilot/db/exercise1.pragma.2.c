...
dout_t lec9Ex2(din_t din[32], dsel_t width);
# 2 "exercise1.c" 2
...
dout_t lec9Ex2(din_t din[32], dsel_t width) {_ssdm_SpecArrayDimSize(din, 32);

 long long *out_accum = malloc (sizeof(long long));
 int* array_local = malloc (64 * sizeof(int));
 int i,j;

  LOOP_SHIFT:for (i=0;i<32 -1; i++) {
    if (i<width)
   *(array_local+i)=din[i];
  else
